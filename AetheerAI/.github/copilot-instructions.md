...
```
User → AetheerAiKernel → Agents → Tools → Result
```
...
| Module | Path | Purpose |
|--------|------|---------|
| `AetheerAiKernel` | `core/aetheerai_kernel.py` | Central controller; boots all subsystems |
...
# Your Copilot Pro models (switch anytime from the AETHEERAI> prompt):
#
