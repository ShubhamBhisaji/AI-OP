...
class ApprovalGate:
    ...
    headless              — set AETHEERAI_HEADLESS=1 or call set_headless(True);
                            auto-rejects (fail-safe — never run unattended risky ops).
    auto_approve          — set AETHEERAI_AUTO_APPROVE=1 for dev/test convenience;
                            logs a warning but proceeds without prompting.
    """

    _headless_default: bool = os.environ.get("AETHEERAI_HEADLESS", "0") == "1"
    _auto_approve_default: bool = os.environ.get("AETHEERAI_AUTO_APPROVE", "0") == "1"
