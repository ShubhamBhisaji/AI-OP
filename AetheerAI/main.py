...
def check_credentials(provider: str) -> None:
    ...
    if choice == "1":
        ...
        os.environ["AETHEERAI_PROVIDER_OVERRIDE"] = "ollama"
...
def pick_provider() -> str:
    ...
    print()
    print("  Tip: Use 'add_api <provider>' inside AETHEERAI to set a key any time.")
    print()
    ...
    _save_to_env("AETHEERAI_DEFAULT_PROVIDER", chosen)
    return chosen
...
def _saved_provider_ready() -> str | None:
    ...
    """
    saved = os.environ.get("AETHEERAI_DEFAULT_PROVIDER", "").strip()
    if not saved:
        return None
    if saved == "ollama":
        return saved
...
def main() -> None:
    ...
    provider = os.environ.get("AETHEERAI_PROVIDER_OVERRIDE", provider)

    kernel = AetheerAiKernel(ai_provider=provider, model=args.model)
